...
const menuItems = [
  ...
  { href: '/inbound',     label: 'Inbound',      icon: Package,         group: 'Operations'   },
  { href: '/putaway',     label: 'Putaway',      icon: Warehouse,       group: 'Operations'   },
  { href: '/inventory',   label: 'Inventory',    icon: Boxes,           group: 'Operations'   },
  { href: '/orders',      label: 'Orders',       icon: ShoppingCart,    group: 'Fulfillment'  },
  { href: '/picking',     label: 'Picking',      icon: ScanLine,        group: 'Fulfillment', liveKey: 'pendingPicks' },
  { href: '/trolleys',    label: 'Trolleys',     icon: Truck,           group: 'Fulfillment'  },
  { href: '/packing',     label: 'Packing',      icon: PackageCheck,    group: 'Fulfillment'  },
  { href: '/shipping',    label: 'Shipping',     icon: Ship,            group: 'Fulfillment'  },
  { href: '/master/bins', label: 'Master Data',  icon: Database,        group: 'Admin'        },
  { href: '/reports',     label: 'Reports',      icon: BarChart3,       group: 'Admin'        },
  { href: '/labels',      label: 'Labels',       icon: Tag,             group: 'Admin'        },
  ...
  return (
    ...
    >
      ...
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-sidebar-border px-3">
        <div className={cn('flex items-center gap-2.5 overflow-hidden min-w-0', isCollapsed && 'justify-center')}>
          <div className="flex size-8 shrink-0 items-center justify-center rounded-xl bg-linear-to-br from-blue-500 to-indigo-600 text-[13px] font-extrabold text-white shadow-md">
            W
          </div>
          {!isCollapsed && (
            <div className="min-w-0">
              <p className="truncate text-[0.8125rem] font-bold text-sidebar-foreground leading-none">WMS Pro</p>
              <p className="truncate text-[0.6875rem] text-sidebar-foreground/45 leading-snug mt-0.5">Warehouse Management</p>
            </div>
          )}
        </div>
        ...
        {!isCollapsed && (
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                className="relative rounded-lg p-1.5 text-sidebar-foreground/50 hover:bg-sidebar-accent hover:text-sidebar-foreground transition-colors"
        ...
        <div className={cn('flex items-center gap-2.5 px-3 py-2.5', isCollapsed && 'justify-center px-2')}>
          ...
          {!isCollapsed && (
            <div className="min-w-0 flex-1">
              <p className="truncate text-[0.75rem] font-semibold text-sidebar-foreground">Administrator</p>
              <p className="truncate text-[0.625rem] text-sidebar-foreground/40">admin@wms.local</p>
            </div>
          )}
          {!isCollapsed && (
            <Tooltip>
              <TooltipTrigger asChild>
                <button className="rounded-md p-1 text-sidebar-foreground/40 hover:text-sidebar-foreground transition-colors">
