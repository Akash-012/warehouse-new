...
function buildCommands(navigate: (href: string) => void): Command[] {
  const nav = (href: string) => () => navigate(href);
  return [
    /* Navigation */
...
interface CommandPaletteProps {
  /** Called with the href when a navigation command is triggered externally */
...
export function CommandPalette({ onNavigate }: CommandPaletteProps) {
  ...
  return (
    /* Backdrop */
    <div
      className="fixed inset-0 z-[200] flex items-start justify-center pt-[15vh] animate-fade-in"
    ...
    >
      <div
        className="w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-popover shadow-xl animate-fade-in-up"
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {cmd.sublabel && (
                          <p className="truncate text-[0.7rem] text-muted-foreground">{cmd.sublabel}</p>
                        )}
                      </div>
